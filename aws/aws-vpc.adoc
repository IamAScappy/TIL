...
===== Private Subnet 생성하기
. 4와 같은 방법으로 추가적으로 서브넷을 하나 더 생성함 
. 대화상자에서는 아래와 같이 입력함
** Name ㅆag: 아이디private1 
** VPC: 생성한 VPC선택 
** Availability Zone: 두번째 AZ 선택 
** CIDR ㅠlock: 10.100.20.0/24

===== Public Routing Table 설정하기
* VPC를 생성하면 기본 라우팅 테이블이 생성됨 
* 기본 라우팅 테이블과는 별도로 각 서브넷에 다른 규칙을 가진 라우팅 테이블을 연결할 수 있음
...
===== 퍼블릭 서브넷과 퍼블릭 라우팅 테이블 연결하기
. 퍼블릭 라우팅 테이블이 선택한 상태에서 아래쪽 탭에서 Subnet Assocations 탭 선택함
. Edit클릭 후 4단계에서 생성한 서브넷 중 퍼블릭 서브넷 (10.100.10.0/24) 의 Associate 체크함
. Save 클릭함

===== 프라이빗 라우팅 테이블 생성 및 프라이빗 서브넷과 연결하기
. 앞 단계와 같은 방식으로 아이디PriRT라우팅 테이블을 생성한다. 단 6.5 ~ 7 단계는 생략하고 추가적인 route 규칙은 생성하지 않음
. 생성한 PriRT 테이블과 프라이빗 서브넷을 7번 과정과 같이 서로 연결함
...
. 잠시 후 생성된 NAT 인스턴스의 Instance ID i-0b0..를 메모장 등에 기록해 놓음
...
===== 프라이빗 라우팅 테이블에 NAT 추가하기
. Services - VPC 선택함
. Route Tables 선택함
. 기존에 생성한 아이디pri 테이블 선택 후 아래쪽 Routes 탭 선택. 반드시 private 라우팅 테이블을 선택함
. Edit 클릭, 아래쪽에 Add another route 클릭함
** Destination: 0.0.0.0/0 Target: NAT인스턴스아이디
. Save 클릭함

===== 프라이빗 인스턴스 생성하기
. 앞 단계와 같은 방식으로 프라이빗 서브넷에 인스턴스를 생성함. 단 아래의 몇 가지 사항을 주의해야 함
** Network: 생성한 VPC 선택함
** Subnet: 프라이빗 서브넷 선택함
** Auto-assign Public IP: Disable Add 
** Tag: Key: Name, Value: 아이디PRI
...
. 생성된 프라이빗 인스턴스의 Private IP를 메모장에 기록해 놓음. Subnet 규칙대로 10.100.20.xxx의 주소를 가지는 것을 확인할 수 있음

===== 프라이빗 인스턴스에 접속하기
* 프라이빗 인스턴스의 경우 Public IP가 따로 없고, 프라이빗 서브넷도 인터넷과 연결되어 있지 않으므로 직접 접속이 불가능함. NAT를 통해 프라이빗 서브넷에 접속해봄
* 맥 터미널에서 아래의 명령을 입력함
...
* NAT 인스턴스의 Bash에서 아래와 같이 입력함. 접속 후 아래처럼 프롬프트를 통해서 프라이빗 인스턴스임을 확인할 수 있음
...
===== 인터넷 테스트하기
* 프라이빗 인스턴스의 Bash에서 아래처럼 입력해봄. 인터넷이 연결되지 않아서 아무것도 나오지 않음
...
* NAT 인스턴스를 선택하고 Actions - Networking - Change Source/Dest. Check 선택, Disable 을 선택함
* 다시 ping 명령을 입력 정상적으로 동작하는지 확인해봄

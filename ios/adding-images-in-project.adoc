...
** 개발자는 기존에 하던 방식대로 iTunes Connect에 풀버전의 번들을 업로드하면 되고, 앱스토어가 애플리케이션이 지원하는 기기에 따라 여러 형태의 번들을 생성함
** 이미지 리소스는 해상도에 따라 분리되고, GPU 리소스는 기기에서 지원하는지에 따라 번들에 나뉨
** 슬라이스된 iOS 애플리케이션은 최신 버전의 아이튠스와 iOS 9.0 버전 이상의 기기만 지원함

* 비트코드
** 컴파일된 프로그램의 중간 형태, 아이튠스 커넥트에 업로드한 애플리케이션이 비트코드라면 앱스토어에서 링크되고 컴파일됨
** iOS 프로젝트에 기본으로 비트코드가 설정되어 있지만, 선택사항임. iOS 애플리케이션을 비트코인으로 제공하려면 번들에 속한 프레임워크와 애플리케이션 모두 비트코인을 지원해야 하며 watchOS는 비트코드가 선택사항이 아닌 필수임

* On-Demand Resources
** 이미지 또는 사운드와 같은 리소스를 키워드로 태그해 놓고 태그를 통해 요청하는 방식
** 앱스토어는 요청에 따라 리소스를 애플 서버에서 관리하거나 다운로드함
** 에를 들어 게임은 레벨이 올라갈 때마다 새로운 리소스를 요청하는데, 앱 내에서 구매하는 아이템은 구매했을 때만 리소스가 다운로드하면 됨

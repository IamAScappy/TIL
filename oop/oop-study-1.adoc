...
public AuthResult authenticate(String id, String pw) {
    ...
    if (member.getVerificationEmailStatus() != 2) {
        return AuthResult.NO_EMAIL.VERIFIED;
    }
...
// #2: 사용할 대상 접근도 추상화
// - 주문 취소 로직은 유지하고 통지 방식을 바꾸는 것이 좋음
public void cancel(String ono) {
    // 주문 취소 처리
...
* 추상화 -> 추상 타입 증가 -> 복잡도 증가
** 클라이언트가 요구하지 않았음에도 '이런 기능이 생길 것이다.' 개발자가 추측한 상태로 추상화를 시도하면 안됨
...
public DropBoxClientFile implements CloudFile {
